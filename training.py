...
class TrainingConfig:
    ...
    eval_batch_size: int = 5  # how many images to sample during evaluation
    num_epochs: int = 200
    gradient_accumulation_steps: int = 1
    learning_rate: float = 2e-5   #for low Datasets
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, eval_dataloader, lr_scheduler, device='cuda'):
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
            model.module.save_pretrained(os.path.join(config.output_dir, "unet"), safe_serialization=False)

            # Save full checkpoint in a unique folder
            ...
            pipeline.save_pretrained(checkpoint_dir, safe_serialization=False)
            
            torch.save({
                'optimizer': optimizer.state_dict(),
                'scheduler': lr_scheduler.state_dict(),
                'epoch': epoch,
            }, os.path.join(config.output_dir, 'train_state.pt'))
            print(f"[Checkpoint] Saved full model to: {checkpoint_dir}")